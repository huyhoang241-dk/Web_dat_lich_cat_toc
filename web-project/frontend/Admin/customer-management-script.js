document.addEventListener('DOMContentLoaded', function() {
    ...
    const cancelAddFormBtn = document.getElementById('cancel-add-customer-form-btn');
    ...
    const cancelEditFormBtn = document.getElementById('cancel-edit-customer-form-btn');
    ...
    const editCustomerRegDateInput = document.getElementById('edit-customer-reg-date'); // Input ngày đăng ký (readonly)


    // Kiểm tra xem các phần tử cần thiết có tồn tại không
    if (searchInput && customerTableBody && showAddFormBtn && addCustomerFormContainer && cancelAddFormBtn && addCustomerForm && editCustomerFormContainer && editCustomerForm && cancelEditFormBtn) {
        ...
        let allCustomers = [ // Sử dụng `let` để có thể cập nhật mảng này sau khi sửa/xóa
            { id: 'KH001', name: 'Nguyễn Văn A', phone: '0912345678', email: 'nguyen.a@example.com', registrationDate: '01/01/2023' },
            { id: 'KH002', name: 'Trần Thị B', phone: '0987654321', email: 'tran.b@example.com', registrationDate: '15/03/2023' },
            { id: 'KH003', name: 'Lê Văn C', phone: '0901112223', email: 'le.c@example.com', registrationDate: '20/05/2023' },
            { id: 'KH004', name: 'Phạm Thị D', phone: '0977778888', email: 'pham.d@example.com', registrationDate: '10/07/2023' },
        ...
        function displayCustomers(customersToDisplay) {
            ...
            customersToDisplay.forEach(customer => {
             ...
             console.log("Customer table updated with", customersToDisplay.length, "customers.");
        }
        ...
        function performSearch() {
            ...
            const filteredCustomers = allCustomers.filter(customer => {
                ...
                return customer.name.toLowerCase().includes(searchTerm) ||
                       ...
                       (customer.email && customer.email.toLowerCase().includes(searchTerm)); // Kiểm tra Email có tồn tại trước khi includes
            });

            // Hiển thị danh sách đã lọc
            displayCustomers(filteredCustomers);
            console.log("Search performed for:", searchTerm, "- Found", filteredCustomers.length, "customers.");
        }
        ...
        addCustomerForm.addEventListener('submit', function(event) {
            ...
            const newCustomerId = 'KH' + (allCustomers.length + 1).toString().padStart(3, '0'); // Tạo ID giả định
            const newCustomer = {
                id: newCustomerId,
                name: customerName,
                phone: customerPhone,
        ...
        customerTableBody.addEventListener('click', function(event) {
            ...
            if (targetBtn) {
                ...
                const customerToEdit = allCustomers.find(customer => customer.id === customerId);
             ...
             if (deleteBtn) {
                 ...
                 if (confirm("Bạn có chắc chắn muốn xóa khách hàng này?")) {
                     ...
                     allCustomers = allCustomers.filter(customer => customer.id !== customerId);
                     // Cập nhật lại bảng hiển thị
                     displayCustomers(allCustomers);
        ...
        console.log("Required elements for customer management logic not found on this page.");
    }
